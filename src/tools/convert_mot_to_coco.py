...
if __name__ == '__main__':
  for split in SPLITS:
    ...
    dsrate=1 #抽帧，要不然数据量太大了

    for seq in sorted(seqs):
      # if '.DS_Store' in seq:
      #   continue
      # if 'myMotData' in DATA_PATH and (split != 'test' and not ('FRCNN' in seq)):
      #   continue
      video_cnt += 1
      out['videos'].append({
        'id': video_cnt,
      ...
      for i in range(num_images):
        if (i < image_range[0] or i > image_range[1]):
          continue
        if i%dsrate!=0 or i==0:
          continue
        image_info = {'file_name': '{}/img1/frame_{}.png'.format(seq, i),
                      'id': image_cnt + i + 1,
                      'frame_id': i + 1 - image_range[0],
                      'prev_image_id': image_cnt + i +1- dsrate if i+1 > dsrate else -1,
                      'next_image_id': \
                        image_cnt + i + 1 + dsrate if  i + dsrate < num_images  else -1,
                      'video_id': video_cnt}
