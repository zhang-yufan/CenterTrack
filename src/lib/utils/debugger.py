  ...
  def add_bird_view(self, dets, vis_thresh=0.3, img_id='bird', cnt=0):
    ...
    for item in dets:
      ...
      if item['score'] > vis_thresh:
        ...
        for e in [[0, 1]]:
          t = 4 if e == [0, 1] else 1
          # print(f'e[0]={e[0]},type:{type(e[0])},'
                # f'rect[e[0]][0]={rect[e[0]][0]},type:{type(rect[e[0]][0])}')
          cv2.line(bird_view, (rect[e[0]][0].astype(np.int32), rect[e[0]][1].astype(np.int32)),
                  (rect[e[1]][0].astype(np.int32), rect[e[1]][1].astype(np.int32)), lc, t,
                  lineType=cv2.LINE_AA)
